<?php
// filepath: c:\wamp64\www\GestionIPCH\app\viewer\plantillasUsuario\headerUsuario.php
require_once './app/sesiones/session.php';
$obj = new Session();
$sesion = $obj->getSession();

// CORRECCIÓN: Verificar si el avatar existe antes de mostrarlo
$rutaAvatarHeader = '/assets/img/user-avatar.svg';

if (!empty($sesion['avatar'])) {
    $avatarPath = $_SERVER['DOCUMENT_ROOT'] . '/' . $sesion['avatar'];
    if (file_exists($avatarPath)) {
        $rutaAvatarHeader = '/' . $sesion['avatar'] . '?t=' . time();
    }
}

// Verificar si es miembro de alabanza
$esAlabanza = (isset($sesion['es_alabanza']) && $sesion['es_alabanza'] == 1);
?>
...
<nav class="navbar">
    ...
    <div class="nav-links">
        ...
        <a href="/misactividades">Actividades</a>
        <?php if($esAlabanza): ?>
            <a href="/misalabanzas">Alabanzas</a>
        <?php endif; ?>
        <a href="/misoraciones">Oraciones</a>
        <a href="/missermones">Sermones</a>
    </div>
    ...
    <div style="display:flex; align-items:center; gap:15px;">
        ...
        <div class="user-menu" id="userMenu" style="cursor:pointer; display:flex; align-items:center;">
            <?php if (!empty($sesion['avatar']) && file_exists($_SERVER['DOCUMENT_ROOT'] . '/' . $sesion['avatar'])): ?>
                <img src="<?php echo $rutaAvatarHeader; ?>" alt="Avatar" class="user-avatar" id="headerAvatarImg" style="object-fit:cover;" />
            <?php else: ?>
                <div class="user-avatar" id="headerAvatarImg" style="background: var(--calypso); display:flex; align-items:center; justify-content:center; color:#000; font-weight:bold; font-size:1.2rem;">
                    <?php echo strtoupper(substr($sesion['nombre'], 0, 1) . substr($sesion['apellido'], 0, 1)); ?>
                </div>
            <?php endif; ?>
            <span class="user-name"><?php echo htmlspecialchars($sesion['nombre']); ?></span>
            ...
            <div class="dropdown-menu" id="dropdownMenu">
                <div style="padding: 10px 20px; border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom:5px;">
                    <span style="display:block; color:#fff; font-weight:bold;"><?php echo htmlspecialchars($sesion['nombre'] . ' ' . $sesion['apellido']); ?></span>
                    <span style="display:block; color:#94a3b8; font-size:0.75rem;">
                        Miembro<?php echo $esAlabanza ? ' | Alabanza' : ''; ?>
                    </span>
                </div>
                <a href="/miperfil"><i class="fas fa-edit" style="margin-right:10px; width:20px; text-align:center;"></i> Mi Perfil</a>
...
<script src="/assets/js/headerUsuario.js?v=<?php echo time(); ?>"></script>
